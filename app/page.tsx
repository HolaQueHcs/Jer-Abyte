...
export default function PanelOperativo() {
  ...
  const cargarStock = useCallback(async () => {
    ...
    if (!error && data) {
      setStockState(data.map((row: any) => ({
        ...
        qty: row.cantidad, precio: parseFloat(row.precio), min: row.minimo,
      })))
    }
  ...
  const setStock = async (updater: StockItem[] | ((prev: StockItem[]) => StockItem[])) => {
    ...
    for (const item of inserts) {
      const { data } = await supabase.from("stock").insert({ nombre: item.nombre, categoria: item.cat, cantidad: item.qty, precio: item.precio, minimo: item.min }).select().single()
      if (data) item.id = data.id
    }
    const updates = newStock.filter(i => {
      if (!i.id) return false
      const orig = prev.find(p => p.id === i.id)
      return orig && (orig.qty !== i.qty || orig.precio !== i.precio || orig.min !== i.min || orig.nombre !== i.nombre || orig.cat !== i.cat)
    })
    for (const item of updates) {
      await supabase.from("stock").update({ nombre: item.nombre, categoria: item.cat, cantidad: item.qty, precio: item.precio, minimo: item.min, updated_at: new Date().toISOString() }).eq("id", item.id)
    }
...
export default function PanelOperativo() {
  ...
  const cargarMetricas = useCallback(async () => {
    ...
    const { data: metData } = await supabase.from("metricas").select("pc_armadas")
    if (metData) {
      setPcArmadas(metData.reduce((s: number, r: any) => s + r.pc_armadas, 0))
    }
  }, [])
  ...
  useEffect(() => {
    cargarStock()
    cargarMetricas()
  }, [cargarStock, cargarMetricas])

  // setStock sincroniza con Supabase automaticamente
  const setStock = async (updater: StockItem[] | ((prev: StockItem[]) => StockItem[])) => {
    const prev = stock
    const newStock = typeof updater === "function" ? updater(prev) : updater
    ...
    for (const item of inserts) {
      const { data } = await supabase
        .from("stock")
        .insert({ nombre: item.nombre, categoria: item.cat, cantidad: item.qty, precio: item.precio, minimo: item.min })
        .select()
        .single()
      if (data) item.id = data.id
    }

    // UPDATEs — items existentes que cambiaron
    const updates = newStock.filter(i => {
      if (!i.id) return false
      const orig = prev.find(p => p.id === i.id)
      return orig && (orig.qty !== i.qty || orig.precio !== i.precio || orig.min !== i.min || orig.nombre !== i.nombre || orig.cat !== i.cat)
    ...
    for (const item of updates) {
      await supabase
        .from("stock")
        .update({ nombre: item.nombre, categoria: item.cat, cantidad: item.qty, precio: item.precio, minimo: item.min, updated_at: new Date().toISOString() })
        .eq("id", item.id)
    }

    // DELETEs — items que ya no están
    const deletes = prev.filter(p => p.id && !newStock.find(n => n.id === p.id))
  ...
  const guardarVenta = async (monto: number, costo: number) => {
    ...
    if (ex) {
      await supabase.from("metricas").update({
        ventas_total: parseFloat(ex.ventas_total) + monto,
        ganancia_total: parseFloat(ex.ganancia_total) + ganancia,
      ...
      <div className="relative z-10">
        <div className="border-b bg-gradient-to-r from-card/90 via-card/80 to-secondary/30 backdrop-blur-sm">
          <div className="mx-auto max-w-5xl px-4 py-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 via-blue-600 to-orange-400 text-white shadow-lg font-bold text-sm ring-2 ring-blue-500/20">
                  JA
                </div>
                <div>
                  <h1 className="text-xl font-bold bg-gradient-to-r from-blue-600 to-blue-800 bg-clip-text text-transparent">Jer Abyte</h1>
        ...
        <div className="mx-auto max-w-5xl px-4 py-5">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="resumen">
              <ResumenTab
                ventas={ventas} gananciaTotal={gananciaTotal} pcArmadas={pcArmadas}
                stockBajo={stockBajo} setVentas={setVentas} setGananciaTotal={setGananciaTotal}
                setTotalVendido={setTotalVendido} setTotalCosto={setTotalCosto}
                totalVendido={totalVendido} totalCosto={totalCosto}
                onNavigate={setActiveTab} onGuardarVenta={guardarVenta}
              />
            </TabsContent>
            <TabsContent value="armado">
              <ArmadoTab
                stock={stock} setStock={setStock} armado={armado} setArmado={setArmado}
                margenGlobal={margenGlobal} setMargenGlobal={setMargenGlobal}
                setPcArmadas={setPcArmadas} onPcArmada={guardarPcArmada}
              />
            </TabsContent>
            <TabsContent value="calculadora"><CalculadoraTab /></TabsContent>
            <TabsContent value="inventario">
