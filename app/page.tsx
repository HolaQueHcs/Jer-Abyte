...
export default function PanelOperativo() {
  ...
  const cargarStock = useCallback(async () => {
    ...
    if (!error && data) {
      setStockState(data.map((row: any) => ({
        ...
        qty: row.cantidad, precio: parseFloat(row.precio), precio_venta: parseFloat(row.precio_venta) || 0, min: row.minimo, nota: row.nota || "", tipo: row.tipo || "real", foto_url: row.foto_url || "",
      })))
    }
  ...
  const setStock = async (updater: StockItem[] | ((prev: StockItem[]) => StockItem[])) => {
    ...
    for (const item of inserts) {
      const { data } = await supabase.from("stock").insert({ nombre: item.nombre, categoria: item.cat, cantidad: item.qty, precio: item.precio, precio_venta: item.precio_venta || 0, minimo: item.min, nota: item.nota || "", tipo: item.tipo || "real", foto_url: item.foto_url || "" }).select().single()
      if (data) item.id = data.id
    }
    const updates = newStock.filter(i => {
      if (!i.id) return false
      const orig = prev.find(p => p.id === i.id)
      return orig && (orig.qty !== i.qty || orig.precio !== i.precio || orig.precio_venta !== i.precio_venta || orig.min !== i.min || orig.nombre !== i.nombre || orig.cat !== i.cat || orig.nota !== i.nota || orig.tipo !== i.tipo || orig.foto_url !== i.foto_url)
    })
    for (const item of updates) {
      await supabase.from("stock").update({ nombre: item.nombre, categoria: item.cat, cantidad: item.qty, precio: item.precio, precio_venta: item.precio_venta || 0, minimo: item.min, nota: item.nota || "", tipo: item.tipo || "real", foto_url: item.foto_url || "", updated_at: new Date().toISOString() }).eq("id", item.id)
    }
