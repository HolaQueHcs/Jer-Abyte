    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {guardando && <span className="text-[10px] text-blue-500 animate-pulse">Guardando en base de datos...</span>}
        </div>
                ...
                <div key={cat}>
                  ...
                  <div className="space-y-2">
                    {items.map(item => {
                      ...
                      const nivel = Math.min(100, Math.round((item.qty / (item.min * 3)) * 100))
                      const estado = item.qty <= item.min ? 'bajo' : item.qty <= item.min * 1.5 ? 'moderado' : 'ok'
                      const color = estado === 'bajo' ? '#ef4444' : estado === 'moderado' ? '#f59e0b' : '#10b981'
                      const isConfirming = confirmDelete === index
                      return (
                        <Card key={index} className="border-0 bg-card/80">
                          <CardContent className="p-3">
                            <div className="flex items-center gap-3">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 flex-wrap mb-1">
                                  <span className="text-xs font-medium">{item.nombre}</span>
                                  {estado === 'bajo' && <Badge variant="destructive" className="text-[8px] px-1.5 py-0">Stock bajo</Badge>}
                              ...
                              <div className="flex flex-col items-center gap-1 min-w-[40px]">
                                <Button
                                  ...
                                  title={isConfirming ? "Clic para confirmar eliminación" : "Eliminar componente"}
                                >
                                  <Trash2 className="h-3.5 w-3.5" />
                                </Button>
                                {isConfirming && (
                                  <span className="text-[8px] text-red-500 text-center leading-tight w-12">¿Confirmar?</span>
                                )}
                              </div>
                            </div>
                          </CardContent>
